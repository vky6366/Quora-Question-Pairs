...
model = Word2Vec.load("Utils\w2v.model")
...
def query_point_creator(q1, q2, model):
    ...
    common = total_words(q1, q2)
    total = total_words(q1, q2)
    input_query.append(common)
    input_query.append(total)
    input_query.append(round(common_words(q1, q2) / total, 2) if total else 0.0)
    input_query.append(round(common / total, 2) if total else 0.0)

    input_query.extend(fetch_token_features(q1, q2))
    input_query.extend(fetch_length_features(q1, q2))
    input_query.extend(fetch_fuzzy_features(q1, q2))  # total = 23

    # Word2Vec part (1200)
    q1_vec = sentence_vector(q1, model).reshape(1, -1)
    q2_vec = sentence_vector(q2, model).reshape(1, -1)
    abs_diff = np.abs(q1_vec - q2_vec)
    avg_vec = (q1_vec + q2_vec) / 2
    w2v_features = np.hstack([q1_vec, q2_vec, abs_diff, avg_vec])  # (1, 1200)

    # Combine all (1223)
    full_vector = np.hstack([np.array(input_query).reshape(1, 23), w2v_features])  # (1, 1223)

    final_vector = full_vector[:, :-1]  
